...
------------------------------------------------------------------------------------------------------------------*/
    ...
    private function __construct($module_id, $slug = false, $is_init = false)
    {
        ...
        $this->_is_network_active = (is_multisite() &&
        ...
        if (is_multisite()) {
             ...
             */
            if (
                false === $this->_storage->get('install_timestamp', false, true) &&
        ...
        if (
            'true' === fs_request_get('fs_clear_api_cache') ||
    ...
    private function maybe_adjust_storage()
    {
        ...
        if (!$is_network_admin) {
            ...
            foreach ($sites as $site) {
                ...
                $is_earlier_install = (!is_null($install_timestamp) &&
    ...
    function _sdk_version_update($sdk_prev_version, $sdk_version)
    {
        ...
        if (
            version_compare($sdk_prev_version, '2.1.0', '<') &&
        ...
        if (
            version_compare($sdk_prev_version, '2.0.0', '<') &&
        ...
        if (
            version_compare($sdk_prev_version, '1.2.3', '<') &&
        ...
        if (
            version_compare($sdk_prev_version, '1.1.5', '<') &&
        ...
        foreach ($connection_status as $slug => $blogs_status) {
            ...
            foreach ($blogs_status as $blog_id => $status_or_user_id) {
                if ('skipped' === $status_or_user_id) {
                    ...
                    if (
                        isset($super_admins[$status_or_user_id]) ||
            ...
            if (!$all_migrated) {
                ...
                foreach ($blogs_status as $blog_id => $status_or_user_id) {
                    ...
                    if (
                        FS_User::is_valid_id($status_or_user_id) ||
    ...
    function _plugins_loaded()
    {
        ...
        if (
            ...
            $this->_storage->is_plugin_new_install = (!is_plugin_active($this->_plugin_basename) &&
                empty($this->_storage->plugin_last_version)
            );
        }
    }
...
?>
    ...
    {
        if (
            !isset($this->_storage->plugin_main_file) ||
    ...
    private function get_module_root_dir_path($module_type = false)
    {
        $is_plugin = empty($module_type) ?
            $this->is_plugin() : (WP_FS__MODULE_TYPE_PLUGIN === $module_type);
    ...
    private function store_id_slug_type_path_map($module_id, $slug)
    {
        ...
        if (
            empty($id_slug_type_path_map[$module_id]['path']) ||
            /**
    ...
    private function get_caller_main_file_and_type()
    {
        ...
        for ($i = 1, $bt = debug_backtrace(), $len = count($bt); $i < $len; $i++) {
            ...
            if (
                !empty($bt[$i]['function']) && in_array($bt[$i]['function'], array(
                    'do_action',
                    'apply_filter',
    ...
    private static function get_plugin_slug($basename)
    {
        ...
        if (is_object(self::$_plugins_info)) {
            if (
                isset(self::$_plugins_info->no_update) &&
                ...
                isset(self::$_plugins_info->response) &&
        ...
        if (
            ...
            switch ($api_result->error->code) {
                case 'curl_missing':
                    $missing_methods = '';
                    if (
                        is_array($api_result->missing_methods) &&
    ...
    function allow_site_tracking()
    {
        ...
        if (
            !$this->is_api_result_entity($result) ||
    ...
    private function parse_settings(&$plugin_info)
    {
        ...
        if ($this->is_network_active() && fs_is_network_admin()) {
            if (
                isset($plugin_info['menu_network']) &&
    ...
    function is_addon()
    {
        return (isset($this->_plugin->parent_plugin_id) &&
                            ...
                            $this->_admin_notices->add(
                                sprintf(
                                    ...
                                    sprintf(
                                        ...
                                        sprintf($this->get_text_x_inline(
...
<?php
    ...
    function _activate_plugin_event_hook()
    {
        ...
        if ($this->is_plugin()) {
            ...
            if (!$this->_is_network_active) {
                 ...
                 * @author Vova Feldman
                 */
                $this->_is_network_active = ($this->_is_multisite_integrated &&
                    fs_is_network_admin()
                );
            }
    ...
    private function get_plugins_data_for_api()
    {
        ...
        if (
            !empty($site_active_plugins_cache->timestamp) &&
        ...
        foreach ($network_plugins as $basename => $data) {
            if (!isset($network_plugins_cache->plugins[$basename])) {
                ...
                $is_site_level_active = (isset($site_active_plugins[$basename]) &&
                    $site_active_plugins[$basename]['is_active']
                );
    ...
    private function get_themes_data_for_api()
    {
        ...
        if (
            !empty($all_cached_themes->timestamp) &&
    ...
    public static function _uninstall_plugin_hook()
    {
        ...
        if (is_object($fs)) {
            ...
            if (
                is_plugin_active($fs->_free_plugin_basename) ||
    ...
    {
        return $this->_plugin->premium_slug;
    }
    ...
    private function set_name($premium_suffix = '')
    {
        ...
        if (is_string($premium_suffix)) {
            ...
            if (!empty($premium_suffix)) {
                ...
                if (
                    strlen($plugin_data['Name']) > $suffix_len &&
    ...
    {
        $label = $this->is_addon() ?
            $this->get_text_inline('Add-On', 'addon') : ($this->is_plugin() ?
    ...
    private function store_file_slug_map()
    {
        ...
        if (
            !isset($file_slug_map[$this->_plugin_basename]) ||
    ...
    function _get_addon_info($addon_id, $is_installed)
    {
        ...
        if (!fs_is_network_admin()) {
            ...
            if (
                $this->is_addon_activated($addon_id) &&
        ...
        if (isset($addon_info['license'])) {
            if (
                isset($addon_storage->subscriptions) &&
    ...
    function is_free_plan()
    {
        ...
        return ('free' === $this->get_plan_name() ||
            !$this->has_features_enabled_license()
        );
    }
     ...
     * @return bool
     */
    function is_whitelabeled_by_flag($ignore_data_debug_mode = true)
    {
        return true;
        ...
        if (!is_null($this->is_whitelabeled)) {
            ...
            if (!$this->has_addons()) {
                ...
                if (!empty($addon_ids)) {
                    ...
                    $is_network_level = (fs_is_network_admin() &&
                    ...
                    foreach ($addon_ids as $addon_id) {
                        ...
                        $network_delegated_connection = ($was_addon_network_activated &&
    ...
    {
        if (
            !isset($this->_storage->subscriptions) ||
        ...
        if (FS_Api::is_api_error($install)) {
            ...
            if (is_object($install)) {
                switch ($install->error->code) {
                    case 'user_exist':
                        $error = ($this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...' .
    ...
    private function activate_license(
        ...
        $is_network_activation_or_migration = (fs_is_network_admin() ||
    ...
    function _network_activate_ajax_action()
    {
        ...
        if (is_array($sites) && !empty($sites)) {
            ...
            if (
                $total_sites === $total_sites_to_delegate &&
    ...
    {
        if (is_null($network)) {
            $network = ($this->_is_network_active &&
    ...
    static function get_site_blog_id(&$site)
    {
        return ($site instanceof WP_Site) ?
            $site->blog_id : (is_object($site) && isset($site->userblog_id) ?
                $site->userblog_id :
                $site['blog_id']);
    }
    ...
    private function update_multisite_data_after_site_deactivation($context_blog_id = 0)
    {
        ...
        if (
            $this->is_install_sync_scheduled() &&
    ...
    {
        return (!empty($this->_storage->beta_data) &&
    ...
    function setup_network_account(
        ...
        if (
            $this->_storage->handle_gdpr_admin_notice &&
    ...
    private function handle_account_connection($installs, $is_site_level)
    {
        ...
        if ($is_site_level) {
            ...
            if (
                !FS_User::is_valid_id($this->_storage->network_user_id) ||
    ...
    function _prepare_admin_menu()
    {
         ...
         * @since  2.0.0
         */
        $should_hide_site_admin_settings = ($this->_is_network_active &&
            !fs_is_network_admin() &&
            !$this->is_delegated_connection() &&
            !$this->is_anonymous() &&
            !$this->is_registered()
    ...
    private function add_submenu_items()
    {
        ...
        if (
            ...
            if (!WP_FS__DEMO_MODE && $this->is_registered()) {
                $show_account = ($this->is_submenu_item_visible('account') &&
                    /**
        ...
        if (
            ...
            if (!WP_FS__DEMO_MODE && !$this->is_whitelabeled()) {
                $show_pricing = ($this->is_submenu_item_visible('pricing') &&
    ...
    private function _fetch_licenses(
        ...
        if (!$is_site_license_synced) {
            ...
            if (is_numeric($site_license_id)) {
                ...
                if (
                    $this->is_api_result_object($api_result, 'licenses') &&
    ...
    function _fetch_payments($plugin_id = false, $flush = false)
    {
        ...
        $include_bundles = (is_object($this->_plugin) &&
    ...
    {
        return (is_object($this->_license) &&
    ...
    private function _sync_plugin_license(
        ...
        if (!$this->has_paid_plan()) {
            ...
            if (
                $site->plan_id != $this->_site->plan_id ||
                ...
                if ($site->is_trial() && (!$this->_site->is_trial() || $site->trial_ends != $this->_site->trial_ends)) {
                    ...
                    if ($is_free && is_null($new_license) && $this->has_any_license() && $this->_license->is_cancelled) {
                        ...
                        $plan_change = $is_free ?
                            ($this->is_only_premium() ? 'activated' : 'upgraded') : (is_object($new_license) ?
                                'changed' :
                                'downgraded');
                    }
                }
    ...
    private function sync_addons($flush = false)
    {
        ...
        $addons = array();
        if (
            $this->is_api_result_object($result, 'plugins') &&
    ...
    private function _handle_account_edits()
    {
        ...
        switch ($action) {
            ...
            case 'downgrade_account':
                ...
                $switch_to_network_install_blog_after_cancellation = (is_numeric($blog_id) &&
                    $plugin_id == $this->get_id() &&
                    !$this->is_trial()
                );
    ...
    function _add_upgrade_action_link()
    {
         ...
         * @since 2.3.0
         */
        $add_upgrade_link = ($add_action_links ||
        ...
        foreach ($user_plugins as $user_plugin) {
            ...
            if (WP_FS__MODULE_TYPE_PLUGIN === $user_plugin->type && !$has_addons) {
                if ($this->_module_id == $user_plugin->id) {
                    ...
                    if (
                        $this->is_api_result_object($addons_result, 'plugins') &&
